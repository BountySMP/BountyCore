...
public class InfoGUI {
    ...
    public InfoGUI(BountyCore plugin) {
        this.plugin = plugin;
    ...
    public void open(Player player) {
        String title = menusConfig.getString("info.title", "§8§lServer Info")
            .replace("&", "§");

        Inventory gui = Bukkit.createInventory(null, 27, title);

        ConfigurationSection itemsSection = menusConfig.getConfigurationSection("info.items");
        if (itemsSection != null) {
            for (String key : itemsSection.getKeys(false)) {
                int slot = itemsSection.getInt(key + ".slot", 0);
                String materialName = itemsSection.getString(key + ".material", "PAPER");
                String name = itemsSection.getString(key + ".name", "§eInfo")
                    .replace("&", "§");
